...
Ewald::Ewald(LAMMPS *lmp, int narg, char **arg) : KSpace(lmp, narg, arg)
{
  ...
  accuracy_relative = fabs(force->numeric(FLERR,arg[0])); // e.g. kspace_style ewald *1e-6*

  kmax = 0;			// to store k-space summation limit
  kxvecs = kyvecs = kzvecs = NULL;
  ug = NULL;		//
  eg = vg = NULL;	//
  sfacrl = sfacim = sfacrl_all = sfacim_all = NULL; // s-factor real,imagine part in each proc and in all respectively

  nmax = 0;			// # of maximum summation 
  ek = NULL;		//
  cs = sn = NULL;	//
...
void Ewald::init()
{
  ...
  // error check
  printf("triclinic support = %d\n",force->kspace->triclinic_support); // ***
  printf("triclinic = %d\n", domain->triclinic);
  printf("criteria = %d\n", domain->triclinic && force->kspace->triclinic_support); 

  triclinic_check(); // in kspace.cpp
  ...
  //printf("before MPI, qsum = %f, qsqsum = %f\n", qsum, qsqsum); // *** 
  ...
  //printf("after MPI, qsum = %f, qsqsum = %f\n", qsum, qsqsum); // *** 

  if (qsqsum == 0.0)
    error->all(FLERR,"Cannot use kspace solver on system with no charge");
  if (fabs(qsum) > SMALL && comm->me == 0) { // charge sum > 1e-5
    char str[128];
  ...
  //printf("accuracy absolute = %f\n", accuracy_absolute); // *** = -1
  // when accuracy_absolute >= 0? by using kspace_modify force arg, arg->accuracy_absolute
  // see L394, kspace.cpp
  if (accuracy_absolute >= 0.0) accuracy = accuracy_absolute;
  else accuracy = accuracy_relative * two_charge_force;
  // two_charge_force see kspace.cpp L64, see update.cpp
 // printf("accuracy_relative = %f\n", accuracy_relative); 
  //printf("accuracy = %f\n", accuracy); // *** 
  ...
  double xprd = domain->xprd;	// Lx
  double yprd = domain->yprd;	// Ly
  double zprd = domain->zprd;	// Lz
  ...
  if (!gewaldflag) {
	...
	//printf("before, g_ewald = %f\n", g_ewald);

    if (g_ewald >= 1.0) g_ewald = (1.35 - 0.15*log(accuracy))/cutoff;
    else g_ewald = sqrt(-log(g_ewald)) / cutoff;
  }
  //printf("after, g_ewald = %f\n", g_ewald);
  ...
  //printf("lprx = %f, lpry = %f, lprz = %f\n",lprx,lpry,lprz); // ***
  //printf("lpr = %f\n",lpr); // ***
  //printf("spr = %f, tpr = %f, estimated_accuracy = %f\n", spr, tpr, estimated_accuracy);// ***
  ...
  // ptr to potential fix
  int ii;
  for (ii = 0; ii < modify->nfix; ii++)
    if (strcmp(modify->fix[ii]->style,"potential") == 0) break;
  if (ii < modify->nfix) fixpot = (FixPot *) modify->fix[ii];
  else fixpot = NULL;

  if (fixpot != NULL) reso_update_flag = 1;
...
void Ewald::setup() // build up k,kmax matrix, and then image out to get each part straight.  ***
{
  // updates K-space resolution if fix potential requires
  // ***** q2 -> kx/y/zmax -> kmax -> eik_dot_r -> summation
  if (reso_update_flag) {
    qsqsum = 0.0;
    for (int i = 0; i < atom->nlocal; i++)
      qsqsum += atom->q[i]*atom->q[i];
  ...
  //printf("kx0 = %f, ky0 = %f, kz0 = %f\n",unitk[0], unitk[1], unitk[2]);

  int kmax_old = kmax;
// see L452-457 in kspace.cpp, kspace_modify kmax/ewald required
  if (kewaldflag == 0) {
	...
	//printf("kxmax = %d, kymax = %d, kzmax = %d, kmax = %d, kmax3d = %d\n",
					//kxmax,kymax,kzmax, kmax, kmax3d);
    
    double gsqxmx = unitk[0]*unitk[0]*kxmax*kxmax;
    double gsqymx = unitk[1]*unitk[1]*kymax*kymax;
    double gsqzmx = unitk[2]*unitk[2]*kzmax*kzmax;

	//printf("gsqxmx = %f, gsqymx = %f, gsqzmx = %f\n", gsqxmx, gsqymx, gsqzmx); // test
	...
	//printf("kxe = %d, kye= %d, kze = %d\n",kx_ewald, ky_ewald, kz_ewald); 
    ...
    kmax = MAX(kxmax,kymax);
    kmax = MAX(kmax,kzmax);
    kmax3d = 4*kmax*kmax*kmax + 6*kmax*kmax + 3*kmax; // cubic extrapolation *** 

    double gsqxmx = unitk[0]*unitk[0]*kxmax*kxmax; // squared kmax in a direction
    double gsqymx = unitk[1]*unitk[1]*kymax*kymax;
    double gsqzmx = unitk[2]*unitk[2]*kzmax*kzmax;
    gsqmx = MAX(gsqxmx,gsqymx); // squared kmax in all directions
    gsqmx = MAX(gsqmx,gsqzmx);
  }
...
------------------------------------------------------------------------- */
// Petersen, J Chem Phys, 103, 3668 (1995) eq.11
...
void Ewald::compute(int eflag, int vflag)
{
  ...
  if (eflag_global) {
    for (k = 0; k < kcount; k++) {
      energy += ug[k] * (sfacrl_all[k]*sfacrl_all[k] + // sfacrl_all: s factor real part; sfacim_all: s factor imagine part
...
void Ewald::eik_dot_r()
{
  ...
  for (ic = 0; ic < 3; ic++) {
    ...
    if (sqk <= gsqmx) {
      ...
      sfacrl[n] = cstr1;		// n = n
      sfacim[n++] = sstr1;		// send sstr1 to sfacim[n], then n = n + 1
    }
  }
